...
`fleetwright/SKILL.md` teaches an agent how to run a fleet: define the work,
enqueue the units, spawn workers with a generic prompt, collect the results.

## Installing it

```bash
fleetwright install-skill          # this project, into .claude/skills/
...
The skill lives at `src/fleetwright/skill/SKILL.md` and ships inside the
